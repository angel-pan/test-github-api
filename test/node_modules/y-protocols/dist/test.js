(function () {
  ...
  const _computeLineSpans = args => {
    ...
    for (; i < args.length; i++) {
      ...
      if (style !== undefined) {
        ...
        if (arg.constructor === String || arg.constructor === Number) {
          // @ts-ignore
          const span = element('span', [create$3('style', mapToStyleString(currentStyle))], [text(arg)]);
          if (span.innerHTML === '') {
            span.innerHTML = '&nbsp;';
          }
          spans.push(span);
  ...
  */
   ...
   * @return {PRNG}
   */
  const create$2 = seed => new DefaultPRNG(seed);
  ...
  const typeListDelete = (transaction, parent, index, length) => {
    ...
    if (length > 0) {
      throw create('array length exceeded')
    }
    if (parent._searchMarker) {
